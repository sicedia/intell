...
# Check if user is logged in to Docker Hub (filter out warnings)
if ! docker info 2>&1 | grep -v "WARNING:" | grep -q "Username"; then
...
# Push backend images
echo -e "${BLUE}Pushing backend:${VERSION_TAG}...${NC}"
...
echo -e "${BLUE}Pushing frontend:${VERSION_TAG}...${NC}"
...
echo ""
echo "To use these images on your server:"
echo "  1. Set IMAGE_TAG=${VERSION_TAG} in .docker.env"
