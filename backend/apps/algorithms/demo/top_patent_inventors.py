"""
Top Patent Inventors algorithm.
Generates horizontal bar chart showing top n inventors by patent publications.
"""
import pandas as pd
import matplotlib
matplotlib.use('Agg')
...
class TopPatentInventorsAlgorithm(BaseAlgorithm):
    """
    Algorithm for generating horizontal bar chart of top patent inventors.
    ...
    def __init__(self):
        ...
        super().__init__(
            algorithm_key="top_patent_inventors",
            algorithm_version="1.0"
        )
        
        self.first_column_name = "Inventors"
    ...
    def _detect_name_column(self, df: pd.DataFrame) -> str:
        """Detect which column contains inventor names."""
        name_keywords = ['inventor', 'name', 'nombre', 'author', 'autor', 'creator']
    ...
    def run(self, dataset: Dataset, params: Dict[str, Any]) -> ChartResult:
        ...
        if df.empty:
            raise ValueError(
                "El dataset está vacío. No se puede generar el gráfico de inventores. "
                "Por favor, verifique que el archivo Excel contenga datos en la hoja de 'Inventors'."
            )
        ...
        if len(df.columns) < 2:
            ...
            raise ValueError(
                ...
                f"Se esperaba: una columna con nombres de inventores y otra con el número de documentos."
            )
        ...
        if name_col is None:
            available_columns = list(df.columns)
            raise ValueError(
                f"No se pudo detectar la columna de nombres de inventores. "
                f"Columnas disponibles: {available_columns}. "
                f"Asegúrese de que el archivo Excel tenga una columna con nombres (ej: 'Inventors', 'Inventores')."
            )
        ...
        if len(df) < top_n:
            raise ValueError(
                f"No hay suficientes inventores en el dataset. "
                f"Se solicitaron los top {top_n}, pero solo hay {len(df)} inventores disponibles."
            )
        
        # Select top n inventors
        ...
        max_val = top_n_inventors[self.second_column_name].max()
        normalized_vals = [
            (x - min_val) / (max_val - min_val) if max_val > min_val else 0.5
            for x in top_n_inventors[self.second_column_name]
        ]
        ...
        top_n_inventors[self.first_column_name] = wrap_labels(top_n_inventors[self.first_column_name], self.text_wrap_width)
        ...
        plt.figtext(-0.1, -0.075,
                    '* Los inventores son las personas físicas que han realizado el acto de creación de la invención descrita en una solicitud de patente. Solo ellos tienen el derecho de ser nombrados como tales en la solicitud.',
                    ha="left", fontsize=12, color="black", wrap=True)
        
        plt.figtext(-0.1, -0.15,
                    '** Un inventor es quien contribuye de manera original y directa a la concepción de la invención y no puede ser una persona jurídica.',
                    ha="left", fontsize=12, color="black", wrap=True)
        ...
        chart_data = {
            ...
            'series': [
                ...
                for _, row in top_n_inventors.iterrows()
            ],
        ...
        return ChartResult(
            ...
            meta={
                ...
                'top_n': top_n,
                'total_inventors': len(df),
                'displayed_inventors': len(top_n_inventors)
            }
        )
