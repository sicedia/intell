...
def generate_horizontal_bar_chart_top_inventors(top_n):
    """
    Generates a horizontal bar chart showing the top n inventors by the number of publications.

    This chart is based on data of patent publications by inventor.
    A color gradient is created to visually represent the range of publications among the highlighted inventors.
    Labels on the bars display both the number of publications and the percentage they represent relative to the total.

    Parameters:
    ----------
    top_n : int
        Number of inventors to display in the chart.
    ...
    sheet_name = "Inventors"
    first_column_name = "Inventors"
    ...
    # Select the top n inventors by the number of publications
    top_n_inventors = df.nlargest(top_n, second_column_name)
    top_n_inventors = top_n_inventors.sort_values(by=second_column_name, ascending=True)
    ...
    min_val = top_n_inventors[second_column_name].min()
    max_val = top_n_inventors[second_column_name].max()
    normalized_vals = [(x - min_val) / (max_val - min_val) if max_val > min_val else 0.5 
                        for x in top_n_inventors[second_column_name]]
    ...
    top_n_inventors[first_column_name] = wrap_labels(top_n_inventors[first_column_name], text_wrap_width)

    # Create the horizontal bar chart
    # Modify inventor names before using them in the chart
    top_n_inventors[first_column_name] = top_n_inventors[first_column_name].apply(lambda x: x.strip().title())

    fig, ax = plt.subplots(figsize=[chart_width, chart_height])
    bars = ax.barh(top_n_inventors[first_column_name], top_n_inventors[second_column_name], color=colors)
    ...
    for i, bar in enumerate(bars):
        num_publications = top_n_inventors[second_column_name].iloc[i]
        percentage = (num_publications / total_publications) * 100  # Calculate percentage
        ax.text(bar.get_width() + 0.025 * max_val, 
    ...
    plt.figtext(-0.1,-0.075, f'* Los inventores son las personas físicas que han realizado el acto de creación de la invención descrita en una solicitud de patente. Solo ellos tienen el derecho de ser nombrados como tales en la solicitud.',
                ha="left", fontsize=12, color="black", wrap=True)

    plt.figtext(-0.1,-0.15, f'** Un inventor es quien contribuye de manera original y directa a la concepción de la invención y no puede ser una persona jurídica.',
                ha="left", fontsize=12, color="black", wrap=True)

    # Save and display the chart    
    plt.savefig(r"C:\Users\francisco.alvarez\Downloads\editable_inventor_chart.png", format='png', bbox_inches='tight')
    plt.savefig(r"C:\Users\francisco.alvarez\Downloads\editable_inventor_chart.svg", format='svg', bbox_inches='tight')
    plt.show()

# Example usage:
generate_horizontal_bar_chart_top_inventors(top_n=10)
